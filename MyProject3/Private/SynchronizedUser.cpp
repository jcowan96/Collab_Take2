...
void USynchronizedUser::BeginPlay()
{
	...
	lastRecordedPosition = VRCamera->GetOwner()->GetTransform().GetTranslation(); //Get transform from parent character, not VRCamera
	lastRecordedRotation = VRCamera->GetComponentTransform().GetRotation();
	lastRecordedScale = VRCamera->GetOwner()->GetTransform().GetScale3D();
...
void USynchronizedUser::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (isControlled)
	{
		...
		if (throttleCounter >= throttleFactor && synchronizationManager.IsValid())
		{
			...
			//Position
			if (VRCamera->GetOwner()->GetTransform().GetTranslation() != lastRecordedPosition)
			{
				if ((VRCamera->GetOwner()->GetTransform().GetTranslation() - lastRecordedPosition).Size() > 0.1f)
				{
					//Log something here
					UE_LOG(LogTemp, Warning, TEXT("[SynchronizedUser] position changed"));

					lastRecordedPosition = VRCamera->GetOwner()->GetTransform().GetTranslation();
			...
			//Scale
			if (VRCamera->GetOwner()->GetTransform().GetScale3D() != lastRecordedScale)
			{
				if ((VRCamera->GetOwner()->GetTransform().GetScale3D() - lastRecordedScale).Size() > 0.1f)
				{
					//Log something here
